...
struct ConvertListView: View {
  @State var topViewModel = TopViewModel(viewModels: [URLInputViewModel(index: 0),
  ...
  var body: some View {
    List {
      TopView(topViewModel: $topViewModel)
      ForEach(0 ..< topViewModel.viewModels.count, id: \.self) { index in
        URLInputView(viewModel: topViewModel.viewModels[index])
      }
    }
  }
}
